...
/**
 * REST controller for managing {@link com.covid19figthers.volunteers.domain.Street}.
 */
@RestController
@RequestMapping("/api")
public class StreetResource {
    ...
    public ResponseEntity<Street> updateStreet(@RequestBody Street street) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, street.getId().toString()))
            .body(result);
    }
    ...
    /**
     * {@code SEARCH  /_search/streets?query=:query} : search for the street corresponding
     * to the query.
     *
